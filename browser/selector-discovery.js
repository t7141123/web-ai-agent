...
const HEURISTICS = {
  ...
  responseBlock: {
    evaluate: (el) => {
      const role =
        el.getAttribute("data-role") || el.getAttribute("role") || "";
...
export class SelectorDiscovery {
  ...
  async discoverAll(page) {
    ...
    this.logger.info(
      `📊 偵測完成: ${found}/${keys.length} 成功` +
        (missing.length ? ` | 失敗: ${missing.join(", ")}` : ""),
    );
  ...
  async _scanCandidates(page, elementType) {
    ...
    for (const sel of candidates) {
      try {
        ...
        if (count > 0) {
          // 對於某些元素，即使當前不可見（例如沒輸入文字時的發送按鈕），只要存在就認領
          if (elementType === 'sendButton' || elementType === 'loadingIndicator') return sel;
...
`;
  ...
  _describeElement(elementType) {
    const descriptions = {
      input:
        "用戶輸入訊息的文字框（Gemini 3 介面）。通常是一個帶有「問問 Gemini 3」預留文字的 contenteditable div 或 textarea，位於頁面底部中央。",
      sendButton:
        "發送訊息的按鈕。通常是一個帶有送出圖示的按鈕，緊鄰輸入框右側。",
      responseBlock:
        "AI 回應的容器元素。包含 AI 生成的文字內容，每次對話後會新增一個。",
      responseText:
        "AI 回應的實際文字內容元素。在回應容器內，包含格式化的 Markdown 文字。",
